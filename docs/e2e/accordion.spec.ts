...
test("SingleOpen: should open first accordion", async ({ page }) => {
  ...
  test("SingleOpen: should open second accordion and close first", async ({ page }) => {
    const allButtons = await page.locator('h3 > button').all()
  ...
  test("SingleOpen: should open first accordion and close second", async ({ page }) => {
    const allButtons = await page.locator('h3 > button').all()
    await allButtons[1].click();
    await page.waitForTimeout(100);
...
test("Customization: should open accordion", async ({ page }) => {
    await page.getByRole("button", { name: "Test accordion with background" }).click();
    await page.waitForTimeout(100);
    ...
    await expect(page).toHaveScreenshot(`${COMPONENT_NAME}-Customization-Open.png`, {
      maxDiffPixelRatio: MAX_DIFF_PIXEL_RATIO,
    });
  });
