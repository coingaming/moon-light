...
const Example = () => {
  ...
  >();
  const [allExpandableRowSet, setAllExpandableRowSet] = useState<
    { [key: string]: boolean }[] | undefined
  >();
  ...
  const toggleAllRowsExpandedState = (allExpanded: boolean) => {
  ...
  useEffect(() => {
    restoreTableState(PREFIX);
  }, []);
  useEffect(() => {
    allRowsExpandedState &&
      setExpandedRows(
        allExpandableRowSet?.map((item) => {
          const key = Object.keys(item)[0];
          return { [key]: true };
        }),
      );
  ...
  useEffect(() => {
    if (allExpandableRowSet) {
      expandedRows &&
        setAllRowsExpandedState(
          allExpandableRowSet.length === expandedRows.length,
        );
      !expandedRows &&
        setAllRowsExpandedState(
          allExpandableRowSet.every((row) => Object.values(row)[0]),
        );
    }
  ...
  const tooltip = (
    tip: string,
    modal: { title: string; panel: () => ReactNode },
  ) => (
    <Tooltip>
      <Tooltip.Trigger>
        <Chip
