...
const Example = () => {
  ...
  const checkIfIndeterminate = (
    ...
    const match = new RegExp(`(^${id}[\\.]|^${id}$)`, "");
    const matches = Object.keys(rowsById).filter(
      (rowId) => match.test(rowId) && rowId !== id,
    );
    return (
      !matches.every((rowId) => selected[rowId] === true) &&
      matches.some((rowId) => selected[rowId] === true)
    );
  };
      ...
      columns: [
        {
          ...
          Cell: ({ row, rowsById }: any) => (
            <div
              ...
              onClick={(e) => {
                if ((e.target as unknown as HTMLElement).tagName === "SPAN")
                  e.stopPropagation();
              }}
            >
              <div className="flex items-center h-full">
